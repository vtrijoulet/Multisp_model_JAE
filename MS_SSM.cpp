  ...
  for(int i = 0; i < sp; i++){
    for(int k = 0; k < n_surv(i); k++){
      ...
      if (sel_model_surv(i,k)==1){ // estimated selectivity at age
        for(int a = 0; a < max_A_surv(i,k); a++){
          s_surv(a,i,k)=1/(1+exp(-logit_s_surv(a,i,k)));
        }
      }
      if (sel_model_surv(i,k)==2){ //logistic selectivity
        //Survey parameters
        A50_surv(i,k)=Aplus(i)/(1+exp(-logit_A50_surv(i,k))); // logit scale for A50 with lower bound =0 and upper band =Aplus
        gamma_surv(i,k)=Aplus(i)/(1+exp(-logit_gamma_surv(i,k))); 
        //Survey selectivity
        for(int a = 0; a < max_A_surv(i,k); a++){
      ...
      for(int t = 0; t < Y; t++){
        ...
        if (flag_Iaa(t,i,k)==1){
          nll_prop_Iaa -= get_acomp_ll(Y, max_A_surv(i,k), Neff_surv(t,i,k), age_comp_model_indices(i), t_paa, t_pred_paa, acomp_pars, index_aref(t,i,k));
          if (data_simulate==1){
            SIMULATE {
              vector<Type> iaa = sim_acomp(Y, max_A_surv(i,k), Neff_surv(t,i,k), age_comp_model_indices(i), t_paa, t_pred_paa, acomp_pars, index_aref(t,i,k));
              for(int a = 0; a < max_A_surv(i,k); a++){
